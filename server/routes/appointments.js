...
router.post('/', auth, [
  ...
  body('type').optional().isIn(['consultation', 'follow-up', 'emergency', 'routine'])
], async (req, res) => {
  try {
    ...
    const { doctorId, date, time, symptoms, type = 'consultation' } = req.body;
...
router.put('/:id/session-phase', auth, authorize('doctor'), [
  body('sessionPhase').isIn(['waiting', 'data-collection', 'initial-assessment', 'examination', 'diagnosis', 'treatment', 'surgery', 'recovery', 'follow-up', 'discharge'])
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
// @access  Private (Doctors)
router.get('/queue/:doctorId/:date', auth, authorize('doctor'), async (req, res) => {
  try {
...
// @access  Private (Doctors)
router.put('/reorder-queue', auth, authorize('doctor'), [
  body('doctorId').isMongoId(),
  body('date').isISO8601(),
  ...
  try {
    ...
    for (const item of appointments) {
      await Appointment.findByIdAndUpdate(item.id, { queuePosition: item.newPosition });
    }
