  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        Array.isArray(req.body.prescription) &&
        req.body.prescription.length > 0
      ) {
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    console.log("Get diagnoses request:", {
      userId: req.user._id,
    ...
    if (req.user.role === "patient") {
      ...
      // Admins can see all diagnoses
      console.log("Admin access - returning all diagnoses");
    }

    console.log("Query for diagnoses:", query);
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    if (
      ...
      diagnosis.patientId._id.toString() !== req.user._id.toString()
    ) {
      return res.status(403).json({ message: "Access denied" });
    }
    ...
    if (req.user.role === "doctor") {
      ...
      if (
        !doctor ||
        diagnosis.doctorId._id.toString() !== doctor._id.toString()
      ) {
        return res.status(403).json({ message: "Access denied" });
      }
    }
