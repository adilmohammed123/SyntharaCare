  ...
  try {
    ...
    if (search) {
      filteredDoctors = doctors.filter(doctor => 
        doctor.userId.profile.firstName.toLowerCase().includes(search.toLowerCase()) ||
        doctor.userId.profile.lastName.toLowerCase().includes(search.toLowerCase()) ||
...
router.get('/:id/availability', async (req, res) => {
  try {
    ...
    for (let time = new Date(startTime); time < endTime; time.setMinutes(time.getMinutes() + slotDuration)) {
      ...
      const isBooked = bookedAppointments.some(appointment => appointment.time === timeString);
...
// @access  Private (Doctors)
router.post('/quick-setup', auth, authorize('doctor'), async (req, res) => {
  try {
    // Check if doctor profile already exists
    const existingDoctor = await Doctor.findOne({ userId: req.user._id });
    if (existingDoctor) {
    ...
    res.status(201).json({
      message: 'Basic doctor profile created successfully. You can now be seen by patients!',
      doctor
    });
  } catch (error) {
...
// @access  Private (Doctors)
router.put('/:id/availability', auth, authorize('doctor'), [
  body('availability').isArray(),
  body('availability.*.day').isIn(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']),
  body('availability.*.startTime').matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/),
  body('availability.*.endTime').matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/)
